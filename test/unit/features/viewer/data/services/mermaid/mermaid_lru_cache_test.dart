...
void main() {
  group('MermaidLruCache', () {
    test(
      'should return null for a key that has never been inserted when the behavior is exercised',
      () {
        final cache = MermaidLruCache<String>(capacity: 4);
    ...
    test(
      'should return the value for a key that was just inserted when the behavior is exercised',
      () {
        final cache = MermaidLruCache<String>(capacity: 4);
    ...
    test(
      ...
      () {
        final cache = MermaidLruCache<String>(capacity: 2);
    ...
    test(
      'should refresh the position of an entry that is re-inserted when the behavior is exercised',
      () {
        final cache = MermaidLruCache<String>(capacity: 2);
    ...
    test(
      'should reject a non-positive capacity at construction when the behavior is exercised',
      () {
        // ArgumentError (not AssertionError) so release builds also
        // refuse to construct an unusable cache; release mode strips
        // asserts entirely.
        expect(
          () => MermaidLruCache<String>(capacity: 0),
          throwsA(isA<ArgumentError>()),
        );
        expect(
          () => MermaidLruCache<String>(capacity: -1),
          throwsA(isA<ArgumentError>()),
        );
      },
    );
