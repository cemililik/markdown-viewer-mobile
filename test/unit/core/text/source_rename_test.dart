...
void main() {
  group('normaliseRenameInput', () {
    test('should returns null for null input (cancelled rename)', () {
      expect(normaliseRenameInput(null), isNull);
    });

    test(
      'should returns null for empty input (clear-the-override sentinel)',
      () {
        expect(normaliseRenameInput(''), isNull);
      },
    );

    test('should returns null for whitespace-only input', () {
      expect(normaliseRenameInput('   \t  '), isNull);
    });

    test('should trims surrounding whitespace from a real label', () {
      expect(normaliseRenameInput('  My Notes  '), 'My Notes');
    });

    test('should passes labels at exactly the cap through unchanged', () {
      final atCap = 'a' * sourceRenameMaxLength;
      expect(normaliseRenameInput(atCap), atCap);
    });

    test('should truncates labels past the cap to the rune-count limit', () {
    ...
    test('should truncation runs on codepoints not UTF-16 code units '
    ...
    test('should rejects whitespace-only payload that survives truncation', () {
